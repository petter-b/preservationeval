...
This module handles the extraction and processing of preservation-related lookup tables 
from JavaScript source code. It processes three types of tables:
...
REGEX_PATTERNS = {
    ...
    'pi_ranges': r'return\s+pitable\[\(\(t<(-?\d+)\s*\?\s*(-?\d+)\s*:\s*t>(\d+)\s*\?\s*(\d+).+?rh<(\d+)\s*\?\s*(\d+)\s*:\s*rh\s*>\s*(\d+)\s*\?\s*(\d+)',
    'mold_ranges': r'if\s*\(t\s*>\s*(\d+)\s*\|\|\s*t\s*<\s*(\d+)\s*\|\|\s*rh\s*<\s*(\d+)\)',
...
def extract_ranges_from_js(js_content: str) -> Dict[TableType, Tuple[Tuple[int, int], Tuple[int, int]]]:
    """Extract temperature and RH ranges for each table from JavaScript code.
    
    Returns a dictionary mapping TableType to ((temp_min, temp_max), (rh_min, rh_max))
    """
    ranges = {}
    ...
    if pi_match:
        t_min, t_min_val, t_max, t_max_val, rh_min, rh_min_val, rh_max, rh_max_val = map(int, pi_match.groups())
        ranges[TableType.PI] = ((t_min_val, t_max_val), (rh_min_val, rh_max_val))
        logger.debug(f"Found PI ranges - T: {t_min_val} to {t_max_val}, RH: {rh_min_val} to {rh_max_val}")
    ...
    if mold_match:
        ...
        logger.debug(f"Found Mold ranges - T: {t_min} to {t_max}, RH: {rh_min} to 100")
...
def create_table_array(values: List[str], dtype: np.dtype, info: TableInfo) -> np.ndarray:
    ...
    """
    data = np.array([dtype(x.strip()) for x in values], dtype=dtype)
...
def fetch_and_validate_tables(url: str) -> Tuple[Dict[TableType, TableInfo], Dict[TableType, np.ndarray]]:
    ...
    try:
        ...
        for table_type, info in table_info.items():
            ...
            if match:
                dtype = np.int32 if table_type in (TableType.PI, TableType.MOLD) else np.float64
                tables[table_type] = create_table_array(
                    match.group(1).split(','),
                    dtype,
                    info
                )
                logger.debug(f"{table_type.value} table processed - shape: {tables[table_type].shape}")
