...
def example_usage():
    ...
    try:
        ...
        table_info, tables = fetch_and_validate_tables("http://www.dpcalc.org/dp.js")  # noqa E501
        ...
        for table_type, info in table_info.items():
            ...
            print(f"Dimensions: {info.temp_size} × {info.rh_size} = {info.total_size}")  # noqa E501
    ...
    for temp, rh in test_points:
        ...
        for table_type, table in tables.items():
            ...
            temp_idx = int(
                (temp - info.temp_min) * (info.temp_size - 1) /
                (info.temp_max - info.temp_min)
            )
            rh_idx = int(
                (rh - info.rh_min) * (info.rh_size - 1) /
                (info.rh_max - info.rh_min)
            )
